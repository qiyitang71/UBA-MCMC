...
public class NBA implements Iterable<NBA_State> {
	...
	/** Returns true if every state has at least one successor for every APElement */
	public boolean isComplete() {
		for (NBA_State state : _index) {
			if (state._edge_manager.getEdgeContainer().size() != _apset.powersetSize()) {
				return false;
			}
			for (Map.Entry<APElement, MyBitSet> edge : state) {
	...
	/**
	 * Convert this (deterministic) NBA to PRISM data structures.
	 */
	public automata.DA<BitSet, AcceptanceRabin> createPrismDAFromDeterministicNBA() throws PrismException
	{
		int numStates = size();

		boolean isComplete = isComplete();
		
		automata.DA<BitSet, AcceptanceRabin> dba;
		dba = new automata.DA<BitSet, AcceptanceRabin>(isComplete ? numStates : numStates + 1);
		int rejectingTrap = -1;
		if (!isComplete) {
			rejectingTrap = numStates;
			// add true self loop
			for (APElement label : new APElementIterator(_apset.size())) {
				dba.addEdge(rejectingTrap, label, rejectingTrap);
			}
		}

		dba.setAcceptance(new AcceptanceRabin());
		BitSet F = new BitSet();
		dba.getAcceptance().add(new RabinPair(new BitSet(), F));

		int i, numLabels, src, dest;
		List<String> apList;
		...
		dba.setAPList(apList);
		// Copy start state
		dba.setStartState(getStartState().getName());
		// Copy edges
		for (i = 0; i < numStates; i++) {
			NBA_State cur_state = get(i);
			src = cur_state.getName();
			if (cur_state.isFinal()) {
				F.set(src);
			}
			for (APElement label : new APElementIterator(_apset.size())) {
				MyBitSet to = cur_state.getEdge(label);
				if (to.isEmpty()) {
					dba.addEdge(src, label, rejectingTrap);
				} else {
					dest = to.nextSetBit(0);
					dba.addEdge(src, label, dest);
					if (to.nextSetBit(dest + 1) != -1) {
						throw new PrismException("NBA is not actually deterministic");
					}
				}
			}
		}
