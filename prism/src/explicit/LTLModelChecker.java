...
public class LTLModelChecker extends PrismComponent
{
	...
	{
		boolean isAtomicProposition = false;
        if (expr.getType() instanceof TypeBool) {
        	if(expr instanceof ExpressionBinaryOp) {
        		ExpressionBinaryOp exprBinOp = (ExpressionBinaryOp)expr;
        		isAtomicProposition = exprBinOp.getOperator() != ExpressionBinaryOp.IMPLIES &&
					exprBinOp.getOperator() != ExpressionBinaryOp.IFF &&
					exprBinOp.getOperator() != ExpressionBinaryOp.OR &&
					exprBinOp.getOperator() != ExpressionBinaryOp.AND;
        	}
        	else if (expr instanceof ExpressionUnaryOp) {
        		ExpressionUnaryOp exprUnOp = (ExpressionUnaryOp)expr;
        		isAtomicProposition = exprUnOp.getOperator() != ExpressionUnaryOp.PARENTH;
        	}
        	if(isAtomicProposition || !getSettings().getBoolean(PrismSettings.PRISM_KEEP_APS)) {
		...
		if ((!getSettings().getBoolean(PrismSettings.PRISM_KEEP_APS) && expr.getType() instanceof TypePathBool) ||
				(getSettings().getBoolean(PrismSettings.PRISM_KEEP_APS) && !isAtomicProposition && (expr.getType() instanceof TypePathBool || expr.getType() instanceof TypeBool))) {
			if (expr instanceof ExpressionBinaryOp) {
				ExpressionBinaryOp exprBinOp = (ExpressionBinaryOp) expr;
	...
	/**
	 * Find the set of states in accepting end components (ECs) in a nondeterministic model wrt a B��chi acceptance condition.
	 * @param model The model
	 * @param acceptance The acceptance condition
	 */
