...
public class PrismSettings implements Observer
{
	...
	public static final String PRISM_UBA_REACH_METHOD				= "prism.ubaReachMethod";
	public static final String PRISM_UBA_POWER						= "prism.ubaPower";
	public static final String PRISM_UBA_VERBOSITY					= "prism.ubaVerbosity";
	public static final String PRISM_UBA_SANITY						= "prism.ubaSanity";
	public static final String PRISM_LTL_STREETT					= "prism.ltlStreett";
	...
	public static final String PRISM_KEEP_APS						= "prism.keepAps";
	...
	public static final Object[][][] propertyData =
	{
		{	//Datatype:			Key:									Display name:							Version:		Default:																	Constraints:																				Comment:
			...
			{ BOOLEAN_TYPE,		PRISM_KEEP_APS,							"Do not summarize Boolean formulas", "4.4",		new Boolean(false), "",
			...
			{ BOOLEAN_TYPE,		PRISM_UBA_PURE,							"Use DBA checking instead of UBA if possible",		"4.3",			new Boolean(false),		"",
																			"Use DBA instead of UBA for LTL model checking (if UBA is actually deterministic)."},
			{ CHOICE_TYPE,		PRISM_UBA_POS_METHOD,					"Method for UBA SCC positivity check",		"4.3",			new String("QR"),		"SVD,QR",
																			"Method for UBA SCC positivity check."},
			{ CHOICE_TYPE,		PRISM_UBA_REACH_METHOD,					"Method for UBA reachability equation system solving",		"4.3",			new String("GS"),		"GS,COLT",
	...
	protected boolean exportDTMCUBAProduct = false;

	public boolean isExportDTMCUBAProduct() {
		return exportDTMCUBAProduct;
	}

	public void setExportDTMCUBAProduct(boolean exportDTMCUBAProduct) {
		this.exportDTMCUBAProduct = exportDTMCUBAProduct;
	}

	public String getExportDTMCUBAProductFilename() {
		return exportDTMCUBAProductFilename;
	}

	public void setExportDTMCUBAProductFilename(String exportDTMCUBAProductFilename) {
		this.exportDTMCUBAProductFilename = exportDTMCUBAProductFilename;
	}

	protected String exportDTMCUBAProductFilename = null;
	...
	public synchronized int setFromCommandLineSwitch(String args[], int i) throws PrismException
	{
		...
		else if (sw.equals("ubapure")) {
			set(PRISM_UBA_PURE, true);
		}
		else if (sw.equals("ubaposmethod")) {
			if (i < args.length - 1) {
				String syntax = args[++i];
				switch (syntax) {
				case "svd":
					set(PRISM_UBA_POS_METHOD, "SVD");
					break;
				case "qr":
					set(PRISM_UBA_POS_METHOD, "QR");
					break;
				default:
					throw new PrismException("Unrecognised option for -" + sw + " switch (options are: svd, qr)");
				}
			} else {
				throw new PrismException("The -" + sw + " switch requires one argument (options are: svd, qr)");
			}
		}
		else if (sw.equals("ubareachmethod")) {
			if (i < args.length - 1) {
				String syntax = args[++i];
				switch (syntax) {
				case "gs":
					set(PRISM_UBA_REACH_METHOD, "GS");
					break;
				case "colt":
					set(PRISM_UBA_REACH_METHOD, "COLT");
					break;
				default:
					throw new PrismException("Unrecognised option for -" + sw + " switch (options are: gs, colt)");
				}
			} else {
				throw new PrismException("The -" + sw + " switch requires one argument (options are: gs, colt)");
			}
		}
		...
		else if (sw.equals("ubasanity")) {
			set(PRISM_UBA_SANITY, true);
		}
		...
		else if (sw.equals("keepAPs")) {
			set(PRISM_KEEP_APS, true);
		} 
		...
		else if (sw.equals("exportproduct")) {
			if (i < args.length - 1) {
				setExportDTMCUBAProduct(true);
				setExportDTMCUBAProductFilename(args[++i]);
			} else {
				throw new PrismException("No file specified for -" + sw + " switch");
			}
	...
	public static void printHelp(PrismLog mainLog)
	{
		...
		mainLog.println("-ubareachmethod <method> ....... Method for UBA reachability: gs, colt");
		mainLog.println("-ubapower ...................... Use power method for SCC probability computations");
		mainLog.println("-ubaverbosity <n>............... Verbosity for UBA checking");
		mainLog.println("-ubasanity ..................... Check whether UBA calculations are sane");
		...
		mainLog.println("-keepAPs ....................... Do not summarize Boolean formulas to APs L0, L1, etc.");
