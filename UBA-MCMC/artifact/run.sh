...
do 
  ...
  if [ "$i" -lt 20 ]; then
    ...
    ../prism/bin/prism -javamaxmem 200g -javastack 1g -timeout 30m -explicit -ltluba -ubaverbosity  1 -ubapower random_lmc.pm -pf  'P=?[ HOA: {"'"UBAs/uba-$i.hoa"'", "sigma" <- "sigma", "pi" <- "pi", "hash" <- "hash", "dollar" <- "dollar"}]'  >> "results/uba-$i.txt"
  fi
