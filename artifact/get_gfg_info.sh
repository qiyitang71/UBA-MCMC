...
# Extract the time in seconds for automata transformation
time_transformation=$(grep -B1 "Automata Transformation Complete!" "$file" | head -n1 | grep -oE '[0-9]*\.[0-9]+|[0-9]+')
...
if [[ ! $time_transformation =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
  time_transformation=0
fi
if [[ ! $time_model_checking =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
    echo "Error: Could not extract valid times from the file."
    exit 1
fi

# Calculate the total time using awk for floating-point addition
total_time=$(awk -v t1="$time_transformation" -v t2="$time_model_checking" 'BEGIN { print t1 + t2 }')

# Output the total time
echo "#states = $gfg, #product = $prod, total_time = $total_time s: $time_transformation (trans), $time_model_checking (mc)"
